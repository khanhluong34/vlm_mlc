...
def main():
    ...
    num_learnable_params = sum(p.numel() for p in trainer.model.parameters() if p.requires_grad)/1e6
    print(f"Number of parameters: {num_params:.2f}")
    print(f"Number of learnable parameters: {num_learnable_params:.2f}")
    if cfg.test:
        test(trainer)
    else:
        train(trainer)
